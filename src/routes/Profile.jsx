        ...
        <div className="flex justify-evenly w-full">
          <div className="border-2 border-slate-200 py-2 px-3 min-h-[100px] w-[40%] rounded-xl">
            ...
            <div className="my-3 flex flex-col justify-center items-center w-full">
              {data.length > 0 ? (
                data.map((item) => <div key={item.mission_id} data-testid={item.mission_id} className="w-[75%] py-3 border-2 border-slate-200 text-left px-4 first:rounded-t-xl last:rounded-b-xl">{item.mission_name}</div>)
              ) : (
